...
def suggested_commands(manifests: list[dict[str, Any]], is_git_repo: bool) -> list[dict[str, Any]]:
    ...
    lockfiles = [item for item in manifests if item.get("kind") in lockfile_kinds]
    if lockfiles:
        for index, item in enumerate(lockfiles, start=1):
            suffix = "" if len(lockfiles) == 1 else f".{index:02d}"
            commands.append(
                {
                    "tool": "osv-scanner",
                    "mode": "manifest_or_lockfile",
                    "command": (
                        f"osv-scanner scan --lockfile={item['path']} --format json "
                        f"--output .audit/osv-results{suffix}.json"
                    ),
                    "reason": "Prefer manifest/lockfile scanning to avoid recursive noise and gitignore warnings, especially for ZIP or non-Git repositories.",
                }
            )
    commands.append(
        {
            ...
            "reason": "Fallback broad scan. In non-Git repositories, gitignore resolution warnings may be noisy; successful JSON output still counts.",
            "recommended": not lockfiles and is_git_repo,
        }
    )
...
def main() -> int:
    ...
    payload = {
        ...
        "suggested_scanners": suggested_commands(manifests, is_git_repo),
        "subagent_task": {
