...
def artifact_uri(run: dict[str, Any], artifact: dict[str, Any]) -> str | None:
    ...
    if isinstance(index, int):
        ...
        if 0 <= index < len(artifacts):
            location = (artifacts[index] or {}).get("location") or {}
            if location.get("uri"):
                return str(location["uri"])
    return None


def first_location(run: dict[str, Any], result: dict[str, Any]) -> dict[str, Any] | None:
    ...
    path = artifact_uri(run, artifact)
    if not path:
        return None
    return {
...
def path_steps(run: dict[str, Any], result: dict[str, Any]) -> list[list[str]]:
...
def main() -> int:
    ...
    for run in sarif.get("runs") or []:
        ...
        for result in run.get("results") or []:
            ...
            location = first_location(run, result)
    ...
    payload = {
        "schema": "graph-reasoning-code-audit/codeql-findings-v1",
        "tool": "codeql",
        "status": "ok",
        "results": normalized,
        "findings": normalized,
