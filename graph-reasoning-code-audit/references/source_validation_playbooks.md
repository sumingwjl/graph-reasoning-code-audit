...
### SCA-to-Source Upgrade Gate

Upgrade an SCA advisory into a source-code vulnerability only when source
validation proves all of these:

- exact vulnerable package and version are present in the runtime dependency
  path, not only a lockfile/dev/test-only context;
- source code reaches the vulnerable API, feature, parser, handler, plugin, or
  transitive behavior named by the advisory;
- an attacker-controlled trigger can reach that usage, or a realistic
  compromised-input path exists for the audited deployment;
- vulnerable configuration, feature flags, framework integration, or
  preconditions are present;
- concrete impact follows from this repository's reachable code path.

If any element is missing, keep the item in the SCA section as
`needs_review`, `not_reachable`, or `SCA-only advisory`. Do not count it as a
confirmed source-code vulnerability.
