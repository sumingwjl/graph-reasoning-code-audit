...
When OSV-Scanner is available, read `dependency_context.json` first and prefer
the `suggested_scanners` command with `mode: "manifest_or_lockfile"`. This is
the default for ZIP downloads and non-Git projects because recursive scanning
can emit noisy gitignore warnings while still producing valid results.

Preferred shape:

```bash
osv-scanner scan --lockfile=/path/to/repo/path/from/dependency_context \
  --format json \
  --output /path/to/repo/.audit/osv-results.json
```

Use recursive scanning as fallback when there is no clear manifest/lockfile or
when the user asks for broad SCA coverage:
...
Treat the JSON output as authoritative for SCA reporting. Do not treat gitignore
resolution warnings alone as scan failure when the JSON contains package and
