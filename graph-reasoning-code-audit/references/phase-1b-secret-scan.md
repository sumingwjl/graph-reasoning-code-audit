...
Check whether the target is a Git repository before running Git-history scan:

```bash
git -C /path/to/repo rev-parse --is-inside-work-tree
```

If this fails, the project may be a ZIP/downloaded source tree. Do not run
`betterleaks git`; run only the current-directory scan and record the Git-history
coverage gap in the phase summary.

Run both Git-history and current-directory scans only when the repo has Git
history:
